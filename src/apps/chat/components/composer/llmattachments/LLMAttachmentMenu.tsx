...
export function LLMAttachmentMenu(props: {
  ...
  const { attachmentDraft: draft, llmSupportsAllFragments, llmSupportsTextFragments, llmTokenCountApprox } = props.llmAttachmentDraft;
  console.log('draft=========', draft);

  const { id: draftId, source: draftSource, input: draftInput, outputsConverting: isConverting } = draft;
  ...
  const handleSetConverterIdx = React.useCallback(
    async (converterIdx: number | null) => {
      return attachmentDraftsStoreApi.getState().toggleAttachmentDraftConverterAndConvert(draftId, converterIdx);
    },
    [draftId, attachmentDraftsStoreApi],
  );

  const handleDeleteOutputFragment = React.useCallback(
    (event: React.MouseEvent, fragmentIndex: number) => {
  ...
  const handleViewImageRefPart = React.useCallback(
    (event: React.MouseEvent, imageRefPart: DMessageImageRefPart) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleViewDocPart = React.useCallback(
    (event: React.MouseEvent, docPart: DMessageDocPart) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  return (
    <CloseableMenu dense placement="top" noTopPadding open anchorEl={props.menuAnchor} onClose={props.onClose} sx={{ minWidth: 260, maxWidth: 460 }}>
      {/* Move Arrows */}
      {!isUnmoveable && (
        <Box sx={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid', borderColor: 'divider' }}>
          <MenuItem disabled={props.isPositionFirst} onClick={handleMoveUp} sx={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
            <KeyboardArrowLeftIcon />
          </MenuItem>
          <MenuItem disabled={props.isPositionLast} onClick={handleMoveDown} sx={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
      ...
      {!isUnconvertible && (
        <ListItem sx={{ fontSize: 'sm', my: 0.75 }}>
          Attach{' '}
          {draftSource.media === 'url'
            ? 'web page'
            : draftSource.media === 'file'
              ? 'file'
              : draftSource.media === 'text'
                ? draftSource.method === 'drop'
                  ? 'drop'
                  : draftSource.method === 'clipboard-read'
                    ? 'clipboard'
                    : draftSource.method === 'paste'
                      ? 'paste'
                      : ''
                : ''}{' '}
          as:
          {uiComplexityMode === 'extra' && (
            <Chip
              ...
              onClick={(event) => handleCopyLabelToClipboard(event, draft.label)}
              sx={{ ml: 'auto' }}
            >
              copy name
            </Chip>
      ...
      {!isUnconvertible &&
        draft.converters.map((c, idx) => (
          ...
          >
            ...
            {c.unsupported ? (
              <Box>
                Unsupported 🤔 <Typography level="body-xs">{c.name}</Typography>
              </Box>
            ) : (
              c.name
            )}
      ...
      {(isInputError || showWarning) && (
        <Box>
          ...
          >
            <ListItemDecorator>{/*<WarningRoundedIcon />*/}</ListItemDecorator>
            <Box>
              <Typography color={isInputError ? 'danger' : 'warning'} level="title-sm">
                {isInputError ? 'Loading Issue' : 'Warning'}
              </Typography>
              {isInputError ? (
                <div>{draft.inputError}</div>
              ) : isUnconvertible ? (
                <div>Attachments of type {draft.input?.mimeType} are not supported yet. You can request this on GitHub.</div>
              ) : isOutputMissing ? (
                <div>File not supported. Please try another format.</div>
              ) : !llmSupportsAllFragments ? (
                <div>May not be compatible with the current model. Please try another format.</div>
              ) : (
                <>Unknown warning</>
              )}
            </Box>
      ...
      {!isInputError && (
        ...
        >
          <ListItemDecorator>{showDetails && canHaveDetails ? <ExpandLessIcon /> : <ExpandMoreIcon />}</ListItemDecorator>
          {!(showDetails && canHaveDetails) ? (
            <Typography sx={{ fontSize: 'sm' }}>Details</Typography>
          ) : (
            <Box sx={{ my: 0.5 }}>
              {/* <- inputs */}
              {showInputs && !!draftInput && (
                <Typography level="body-sm" textColor="text.primary" startDecorator={<AttachmentIcon sx={indicatorSx} />}>
                  {draftInput.mimeType}
                  {typeof draftInput.dataSize === 'number' ? ` · ${draftInput.dataSize.toLocaleString()} bytes` : ''}
                </Typography>
              )}
              ...
              {showInputs && !!draftInput?.urlImage && (
                <Typography level="body-sm" sx={indicatorGapSx}>
                  {draftInput.urlImage.mimeType} · {draftInput.urlImage.width} x {draftInput.urlImage.height} ·{' '}
                  {draftInput.urlImage.imgDataUrl?.length.toLocaleString()}
                  {' · '}
                  <Link
                    onClick={(event) => {
                      event.preventDefault();
              ...
              <Box sx={{ mt: 1 }}>
                {isOutputMissing ? (
                  <Typography level="body-sm" startDecorator={<ReadMoreIcon sx={indicatorSx} />}>
                    ...
                  </Typography>
                ) : (
                      ...
                      return (
                        <Typography key={index} level="body-sm" sx={{ color: 'text.primary' }} startDecorator={<ReadMoreIcon sx={indicatorSx} />}>
                          <span>
                            ...
                            {part.dataRef.reftype === 'dblob' ? part.dataRef.bytesSize?.toLocaleString() || 'no size' : '(remote)'} ·&nbsp;
                          </span>
                          <Chip
                            component="span"
                            size={isOutputMultiple ? 'sm' : 'md'}
                            color="primary"
                            variant="outlined"
                            startDecorator={<VisibilityIcon />}
