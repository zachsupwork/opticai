...
export function LLMAttachmentsList(props: {
  ...
  const itemMenuAttachmentDraft = itemMenuAttachmentDraftId
    ? llmAttachmentDrafts.find((la) => la.attachmentDraft.id === draftMenu.attachmentDraftId)
    : undefined;
  ...
  const handleOverallMenuToggle = React.useCallback(
  ...
  const handleOverallClear = React.useCallback(async () => {
    if (
      await showPromisedOverlay('chat-attachments-clear', { rejectWithValue: false }, ({ onResolve, onUserReject }) => (
        <ConfirmationModal
          open
  ...
  const handleDraftMenuToggle = React.useCallback(
    (attachmentDraftId: AttachmentDraftId, anchor: HTMLAnchorElement) => {
      handleOverallMenuHide();
  ...
  const handleDraftAction = React.useCallback(
    (attachmentDraftId: AttachmentDraftId, actionId: LLMAttachmentDraftsAction) => {
    ...
    <>
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        <Box sx={{ height: '100%', pr: 5, overflowX: 'auto', display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {(agiAttachmentPrompts.isVisible || agiAttachmentPrompts.hasData) && <LLMAttachmentsPromptsButtonMemo data={agiAttachmentPrompts} />}
      ...
      {!!viewerImageRefPart && <ViewImageRefPartModal imageRefPart={viewerImageRefPart} onClose={handleCloseImageViewer} />}

      {/* Text Viewer Modal */}
      {!!viewerDocPart && <ViewDocPartModal docPart={viewerDocPart} onClose={handleCloseDocPartViewer} />}
